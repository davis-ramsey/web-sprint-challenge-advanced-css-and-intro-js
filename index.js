...
// 		bio: 'He joined lambda school to learn web development. Building beautiful web applications is his passion!'
// 	})
// );
...
function lotsOfArt(artists) {
	const filteredArray = [];
	for (let artist of artists) if (artist.paintings > 100) filteredArray.push(artist.name);
...
function getHTML(data) {
	...
	for (let item of data) {
		for (let key in item) {
			if (key === 'name') {
				const name = item[key].split(' ').join('-');
				html += `<div id="artist"><div class="image"><img src="assets/${name}.jpg"/></div><div class = "name">
        <a href="${item.wikipedia}">${item[key]}</a> </div><div class = "bio">${item.bio}</div></div>`;
			}
		}
	}
	return html;
...
function randomize(array) {
	const randomNums = [];
	while (randomNums.length < array.length) {
		const random = Math.floor(Math.random() * array.length);
		if (randomNums.indexOf(random) === -1) randomNums.push(random);
	}
	const randomArray = [];
	for (let num of randomNums) randomArray.push(artists[num]);
	return randomArray;
}
...
function newGet20s(array) {
	return array.filter((item) => {
		const dates = item.years.split(' - ');
		if (dates[0] >= 1900 && dates[1] < 2000) return true;
	});
}
